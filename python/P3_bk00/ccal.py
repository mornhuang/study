...
programs under UNIX X-Windows.
"""

#Remember, in this program:
#   month=0 means January, month=1 means February ...;
...
zodiacEn = ["Mouse", "Ox", "Tiger", "Rabbit", "Dragon", "Snake",
            "Horse", "Goat", "Monkey", "Rooster", "Dog", "Pig"]
zodiacGB = ["鼠", "牛", "虎", "兔", "龙", "蛇", "马", "羊", "猴", "鸡",
            "狗", "猪"]
...
def solarDaysFromBaseYear(d):    # d is a Date class
    ...
    for x in range(d.month):
        offset += daysInSolarMonth[x]
...
def lunarDate2GB(dLunar, isLeapMonth):
    ...
    if tmp in lunarHoliday:
        ...
        return num2GB(int(dLunar.day + 1))
        ...
        if iDay == 0:
            ow('始于 阴历%s年%s%s月 (%s%s年, 生肖属%s)\n' %
               (num2GB(dLunar.year + 1900), '闰' * isLeapMonth,
                ...
                ganGB[dLunar.gan], zhiGB[dLunar.zhi], zodiacGB[dLunar.zhi]))
            ow('=' * 74 + '\n')
            for n in range(7):
                ow("%3s %2s     " % (weekdayEn[n][:3], weekdayGB[n]))
            ow('\n\n')
            for n in range(int(dLunar.weekday)):
                ow(' ' * 11)
