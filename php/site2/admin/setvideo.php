...
elseif($action == 'search')
{
	...
	$url = "$admin_file?adminjob=setvideo&action=$action&author=" . urlencode($author) . "&cid=$cid&nid=$nid&subject=" . urlencode($subject) . "&synopsis=" . urlencode($synopsis) . "&playactor=" . urlencode($playactor) . "&director=" . urlencode($director) . "&hits=$hits&best=$best&serialise=$serialise&postdate=$postdate&orderway=$orderway&asc=$asc&lines=$lines&";
