...
if(!$action)
{
	...
	while($rt = $db->fetch_array($query))
	{
		...
		$config['stime'] = get_date($config['stime'], 'Y-m-d H:i:s');
		$config['etime'] = get_date($config['etime'], 'Y-m-d H:i:s');
		$rt['config'] = $config;
		$advertdb[] = $rt;
	}
}
elseif($action == 'add' || $action == 'edit')
{
	...
	elseif($step == '2')
	{
		...
		$cfg = array();
		if(!$descrip || !$code || !$ckey || !$stime || !$etime) adminmsg('operate_fail');
		switch($ckey)
		{
			case 'header':
			case 'footer':
		...
		$stime = strtotime($stime) - $db_timedf * 3600;
		$etime = strtotime($etime) - $db_timedf * 3600;
